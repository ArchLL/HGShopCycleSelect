...
@interface HGShopStartTimeView ()

/// 选择送货时间背景
@property (nonatomic, strong) UIView *startBgView;
/// 选择还货时间背景
@property (nonatomic, strong) UIView *endBgView;
/// 使用周期的开始日期  年-月-日
...
/// 记录上一次选中的送货日期按钮
@property (nonatomic, strong) UIButton *startLastDateBtn;
/// 记录上一次选中的送货时间按钮
@property (nonatomic, strong) UIButton *startLastTimeBtn;
/// 记录上一次选中的还货时间按钮
@property (nonatomic, strong) UIButton *endLastTimeBtn;
/// 最新网络时间(更新) (年-月-日 时:分:秒)
@property (nonatomic, copy) NSDate *networkDate;
/// 最新网络时间(更新) (年-月-日 时:分:秒)
@property (nonatomic, copy) NSString *networkDateStr;
/// 送货底部时间间隔背景
@property (nonatomic, strong) UIView *startBottomTimeView;
/// 送货时间数组
@property (nonatomic, copy) NSArray *startTimeArr;
/// 今日时间(年月日)
@property (nonatomic, copy) NSString *todayDateStr;
/// 明日日期(年月日)
@property (nonatomic, copy) NSString *tomorrowDateStr;

@end

@implementation HGShopStartTimeView
...
- (instancetype)initWithFrame:(CGRect)frame NetworkDate:(NSDate *)networkDate RentDays:(NSInteger)days StartDate:(NSString *)startDate StartTime:(NSString *)startTime EndDate:(NSString *)endDate EndTime:(NSString *)endTime {
    ...
    if (self) {
        ...
        self.networkDateStr = [HGShopTimeSelectTool getDetailStringWithDate:networkDate];
        self.todayDateStr = [HGShopTimeSelectTool getSimpleStringWithDate:networkDate];
        self.tomorrowDateStr = [HGShopTimeSelectTool updateDateWithDate:self.networkDate Interval:1 * 24 * 60 * 60 IsAdd:YES];
        //送货
...
- (void)creatContentViewWithIsStart:(BOOL)isStart {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (isStart) {
            ...
            //文本
            btn.tag = 103;
            attriStr = [[NSMutableAttributedString alloc] initWithString:self.endDate];
    ...
    if (isStart) {
        ...
        if ([self.startDate isEqualToString:self.tomorrowDateStr]) {
            //缓存日期是明日
            //更新日期
        ...
        }else if ([self.startDate isEqualToString:@"quick"]) {
            //缓存日期是极速达
            //更新日期
            ...
            //时间段
            //时间判断
            if ([self isGreaterThanWithCurrentDate:self.networkDate ToDate:@"7:00"]) {
                NSLog(@"7:00之后");
...
- (void)createTimePartViewWithView:(UIView *)view TimeArr:(NSArray *)timeArr IsStart:(BOOL)isStart IsFrist:(BOOL)isfrist {
    ...
    if (isStart && !isfrist) {
        //更换送货背景的Height
        CGRect frame1 = self.startBgView.frame;
        if (timeArr.count > 3) {
            frame1.size.height = 250;
        }else {
            frame1.size.height = 200;
        }
        self.startBgView.frame = frame1;
        ...
        //更改换货背景的Y值
        CGRect frame3 = self.endBgView.frame;
        if (timeArr.count > 3) {
            frame3.origin.y = 250;
        }else {
            frame3.origin.y = 200;
        }
        self.endBgView.frame = frame3;
    ...
    for (int i = 0; i < timeArr.count; i++) {
        ...
        if (isStart) {
            ...
            if (self.endTime) {
                //设置初始状态(上个界面的缓存)
                if (self.endTime == timeBtn.titleLabel.text) {
                    timeBtn.backgroundColor = HexRGB(0xc42d6c);
                    [timeBtn setTitleColor:[UIColor whiteColor] forState:(UIControlStateNormal)];
                    self.endLastTimeBtn = timeBtn;
                }
            }
        }
...
- (void)refreshStartTimePart {
    ...
    [HGShopTimeSelectTool getNetWorkDateSuccess:^(NSDate *networkDate, NSString *networkDateStr) {
        ...
        [strongSelf.startBottomTimeView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
        strongSelf.startLastTimeBtn = nil;
        //得到当前时间
        strongSelf.networkDate = networkDate;
        strongSelf.networkDateStr = networkDateStr;
        strongSelf.todayDateStr = [HGShopTimeSelectTool getSimpleStringWithDate:strongSelf.networkDate];
        strongSelf.tomorrowDateStr = [HGShopTimeSelectTool updateDateWithDate:strongSelf.networkDate Interval:24 * 60 * 60 IsAdd:YES];
        //判断时间
        NSArray *timeArr = [NSArray array];
        if (strongSelf.startLastDateBtn.tag - 100 == 0) {
            //今日
            if ([strongSelf isGreaterThanWithCurrentDate:networkDate ToDate:@"7:00"]) {
                NSLog(@"7:00之后");
                timeArr = @[@"15:00-17:00", @"17:00-19:00", @"19:00-22:00"];
            } else {
                NSLog(@"7:00之前");
        ...
        } else if(strongSelf.startLastDateBtn.tag - 100 == 1){
            //明日
            timeArr = @[@"10:00-12:00", @"12:00-14:00", @"14:00-16:00", @"16:00-18:00", @"18:00-20:00", @"20:00-22:00"];
        } else {
            //极速达
        ...
        strongSelf.startTimeArr = timeArr;
        //更新送货时间间隔(重新添加按钮)
        [strongSelf createTimePartViewWithView:strongSelf.startBottomTimeView TimeArr:timeArr IsStart:YES IsFrist:NO];
    }];
}
...
- (void)dateBtnAction:(UIButton *)sender {
    ...
    //2.更改上一个按钮的显示状态
    NSMutableAttributedString *attriStr;
    if (self.startLastDateBtn) {
        self.startLastDateBtn.backgroundColor = [UIColor whiteColor];
        switch (self.startLastDateBtn.tag - 100) {
            case 0: //今日
                attriStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@\n(今日)", self.todayDateStr]];
                [attriStr setAttributes:@{NSForegroundColorAttributeName:HexRGB(0x333333)} range:NSMakeRange(0, attriStr.length)];
                [attriStr setAttributes:@{NSForegroundColorAttributeName:HexRGB(0x999999), NSFontAttributeName:NFont(12)} range:NSMakeRange(attriStr.length - 4, 4)];
        ...
        [self.startLastDateBtn setAttributedTitle:attriStr forState:(UIControlStateNormal)];
    }
...
- (void)updateBtnStatus:(UIButton *)sender {
    ...
    if (index < 6) {
        //送货
        if (self.startLastTimeBtn) {
            self.startLastTimeBtn.backgroundColor = [UIColor clearColor];
            [self.startLastTimeBtn setTitleColor:HexRGB(0x999999) forState:(UIControlStateNormal)];
        }
        sender.backgroundColor = HexRGB(0xc42d6c);
        [sender setTitleColor:[UIColor whiteColor] forState:(UIControlStateNormal)];
        self.startLastTimeBtn = sender;
        //回调
        if (self.startTimeSelectBlock) {
            //三小时极速达特殊处理 7:00-16:00
            if ([sender.titleLabel.text isEqualToString:@"三小时极速达"]) {
                //根据送货时间往后延长3个小时
                NSString *endTimeStr = [HGShopTimeSelectTool updatePreciseDateWithDate:self.networkDate Interval:3*60*60 IsAdd:YES];
                NSString *tempStr = [NSString stringWithFormat:@"%@_%@", self.networkDateStr, endTimeStr];
        ...
        if (self.endLastTimeBtn) {
            self.endLastTimeBtn.backgroundColor = [UIColor clearColor];
            [self.endLastTimeBtn setTitleColor:HexRGB(0x999999) forState:(UIControlStateNormal)];
        }
        sender.backgroundColor = HexRGB(0xc42d6c);
        [sender setTitleColor:[UIColor whiteColor] forState:(UIControlStateNormal)];
        self.endLastTimeBtn = sender;
        //回调
...
- (void)timeBtnAction:(UIButton *)sender {
    ...
    NSString *toTime;
    //1.获取时间间隔
    if (self.startLastDateBtn.tag == 102) {
        //三小时极速达
        fromTime = @"7:00";
        toTime = @"16:00";
    }else {
    ...
    if (sender.tag - 200 < 6) {
        ...
        if (self.startLastDateBtn.tag == 101) {
            //明日
            ...
            [HGShopTimeSelectTool getNetWorkDateSuccess:^(NSDate *networkDate, NSString *networkDateStr) {
                ...
                strongSelf.todayDateStr = [HGShopTimeSelectTool getSimpleStringWithDate:strongSelf.networkDate];
                strongSelf.tomorrowDateStr = [HGShopTimeSelectTool updateDateWithDate:strongSelf.networkDate Interval:24 * 60 * 60 IsAdd:YES];
                if (strongSelf.startLastDateBtn.tag == 102) {
                    //极速达 7:00-16:00可选
                    //判断时间间隔是否可选
                    if ([strongSelf isOpenTimeWithFromDate:fromTime toDate:toTime]) {
                        [strongSelf updateBtnStatus:sender];
                    } else {
                    ...
                    if ([strongSelf isGreaterThanWithCurrentDate:networkDate ToDate:@"7:00"]) {
                        //7点之后
                        if ([strongSelf isGreaterThanWithCurrentDate:networkDate ToDate:@"12:00"]) {
                            //12:00之后三个时间段不可选
                            //判断当前时间是不是16:00之前，如果是16:00之前，提醒去选择极速达
                            if ([strongSelf isGreaterThanWithCurrentDate:networkDate ToDate:@"16:00"]) {
                                //16:00之后
...
- (void)updateReturnGoodsBtnTitleWithIsAdd:(BOOL)isAdd {
    ...
    //当前时间 年-月-日
    NSString *currentDate = [HGShopTimeSelectTool getSimpleStringWithDate:self.networkDate];
    //判断当前送货时间比当前时间多几天
    NSInteger intervalDays = [HGShopTimeSelectTool getTimeDifferenceWithFromDate:self.startDate toDate:currentDate];
    //获取最新租赁时长
    NSInteger newDays;
    if (isAdd) {
        newDays = self.days + intervalDays;
    }else {
        newDays = self.days - intervalDays;
    }
    //获取新的还货时间
    NSString *newEndDate = [HGShopTimeSelectTool updateDateWithDateStr:currentDate Interval:newDays * 24 * 60 * 60 IsAdd:YES];
...
- (BOOL)isOpenTimeWithFromDate:(NSString *)fromDate toDate:(NSString *)toDate {
    ...
    NSString *dateStr = [formatter stringFromDate:self.networkDate];
    //开始时间和结束时间
    ...
    NSTimeInterval currentTime = [self.networkDate timeIntervalSince1970];
...
- (BOOL)judgeWeatherSelectTime {
    if (!self.startLastTimeBtn && !self.endLastTimeBtn) {
        [MBProgressHUD showError:@"请选择送货时间和还货时间" toView:nil];
        return NO;
    }else if(!self.startLastTimeBtn) {
        [MBProgressHUD showError:@"请选择送货时间" toView:nil];
        return NO;
    }else if(!self.endLastTimeBtn){
